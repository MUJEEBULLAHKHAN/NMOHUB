...
@Component({
  selector: 'app-feas-dashboard',
  standalone: false,
  //imports: [],
  providers: [DisplaymessageComponent],
  templateUrl: './feas-dashboard.component.html',
  styleUrl: './feas-dashboard.component.scss'
})


export class FeasDashboardComponent {
  ...
  DashboardCountersByUser() {
    this.feasService.DashboardCountersByUser(this._userInfo.employeeId).subscribe({
      next: (user) => {
  ...
  GetAllDashboardCounter() {
    this.feasService.GetAllDashboardCounter().subscribe({
      next: (user) => {
        if (user.body.success) {
          this.status = user.body.data;
        }
