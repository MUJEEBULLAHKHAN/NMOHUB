...
@Component({
  ...
  styleUrl: './mvp-dashboard.component.scss'
})


export class MVPDashboardComponent {
  ...
  DashboardCountersByUser() {
    this.mVPService.DashboardCountersByUser(this._userInfo.employeeId).subscribe({
      next: (user) => {
  ...
  GetAllDashboardCounter() {
    this.mVPService.GetAllDashboardCounter().subscribe({
      next: (user) => {
        if (user.body.success) {
          this.status = user.body.data;
        }
