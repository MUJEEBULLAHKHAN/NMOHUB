...
@Component({
  selector: 'app-service-status-request',
  standalone: false,
  //imports: [],
  providers : [DisplaymessageComponent],
  templateUrl: './service-status-request.component.html',
  styleUrl: './service-status-request.component.scss'
})
export class ServiceStatusRequestComponent {
...
public togglePassword() {
  ...
  if(this.roleId == 10) {
this.GetAllProjectRequests(statusId);
  }
  if(this.roleId == 1) {
    this.GetProjectRequestsByEmployeeId(statusId);
  }
  
  }
generateReport() {
    const statusId = 0; // You can change this or get from filter
    this.reportService.exportIncubatorList(statusId).subscribe((blob: Blob) => {
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `Incub_${new Date().toISOString().replace(/[:.]/g, '_')}.csv`;
      a.click();
      window.URL.revokeObjectURL(url);
    });
  }
GetAllProjectRequests(statusId: any) {

    this.authservice.GetAllProjectRequests(statusId).subscribe( {
     next:(x)=>{
       if (x.ok == true) {
  ...
  GetProjectRequestsByEmployeeId(statusId: any) {

    this.authservice.GetProjectRequestsByEmployeeId(this.employeeId, statusId).subscribe( {
     next:(x)=>{
       if (x.ok == true) {
        this.projectRequestList = x.body.data;
      }
